...
def plot_artifact(artifact_path: Path, output_path: Path) -> Path:
    ...
    figure = make_subplots(
        ...
        subplot_titles=("Aligned continuous pitch", "Absolute pitch error"),
    )
    figure.add_trace(
    ...
    figure.update_xaxes(title_text="Reference time (seconds)", row=2, col=1)
    figure.update_layout(title="Vocalika feasibility analysis", hovermode="x unified", height=850)
